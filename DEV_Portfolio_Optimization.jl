...
ENV["PYTHON"] = "/home/felhasznalo/Portfolio/Optimization/venv/bin/python"
ENV["GKSwstype"] = "100"
...
const BENCHMARK = "SPY"
const ENABLE_PLOTTING = false  # Set to true to enable plotting in all stages

# === STAGE 1: APSO PARAMETERS ===
const APSO_BOOTSTRAP_SAMPLES = 300
const APSO_SWARM_PARTICLES = 200
const APSO_ITERATIONS = 50
const APSO_MULTI_STARTS = 10
...
const FE_OPTUNA_TRIALS = 30
const RFE_N_ROUNDS = 30
...
const FC_RIDGE_ALPHA_END = 4     # Exponent for logspace end (10^4)
const FC_RIDGE_ALPHA_COUNT = 100 # Number of alphas to generate
const FC_RIDGE_CV_FOLDS = 5
const FC_NOMAD_MAX_EVALS = 100
const FC_BOOTSTRAP_SAMPLES = 300
...
function calculate_transaction_cost(current_weights::Vector, previous_weights::Vector; buy_cost = 0.0005, sell_cost = 0.0005)
	...
	buys = sum(delta_w[delta_w .> 0])
	sells = sum(abs.(delta_w[delta_w .< 0]))
...
function multi_start_pso(objective_function, bounds, n_particles, dimensions, max_iter, n_starts,
	...
	for start in 1:n_starts
		println("\nStarting Adaptive PSO Run $start/$n_starts")
		...
		for i in 1:max_iter
			...
			if i % 10 == 0 || i == max_iter
				@info "Run $start, Iteration $i: Best Cost=$(round(optimizer.swarm.best_cost, digits=4)), w=$(round(w, digits=4)), c1=$(round(c1, digits=4)), c2=$(round(c2, digits=4))"
			end
		end
...
function create_summary_distribution_plot(data, title_suffix, color)
	...
	if !isempty(numeric_data)
		...
		# Add invisible plots to insert text statistics into the legend
...
function run_apso_stage(market_quarterly, prices_weekly;
	...
	if "Date" ∉ names(prices_weekly)
		rename!(prices_weekly, names(prices_weekly)[1] => "Date");
	end
	...
	if benchmark_col_name ∉ names(prices_weekly)
		println("ERROR: Benchmark '$(benchmark_col_name)' not found in price data.")
		return nothing, nothing, 0, 0;
	end
	println("Using benchmark: $(benchmark_col_name)")

	stock_cols = [col for col in names(prices_weekly) if col != benchmark_col_name && col != "Date"]
	if isempty(stock_cols)
		return nothing, nothing, 0, 0;
	end
	log_ret(p) = [NaN; log.(p[2:end] ./ p[1:(end-1)])]
	clean_val(x) = !isfinite(x) ? 0.0 : x
	stock_weekly_log_returns = DataFrame(Date = prices_weekly.Date)
	for col in stock_cols
		prices = coalesce.(prices_weekly[:, col], NaN)
		stock_weekly_log_returns[!, col] = clean_val.(log_ret(prices))
	end
	benchmark_prices = coalesce.(prices_weekly[:, benchmark_col_name], NaN)
	rsp_weekly_log_returns = DataFrame(Date = prices_weekly.Date, Benchmark = clean_val.(log_ret(benchmark_prices)))
	for col in stock_cols
		stock_weekly_log_returns[!, col] = winsorize_series(stock_weekly_log_returns[!, col]);
	end
	rsp_weekly_log_returns.Benchmark = winsorize_series(rsp_weekly_log_returns.Benchmark)

	# --- Main Loop Setup ---
	all_optimal_weights = Vector{Float64}[]
	all_information_ratios = Float64[]
	valid_quarters = Date[]
	bootstrap_results = Dict()
	market_quarterly_sorted = sort(market_quarterly, :Date)
	quarters_to_process = unique(firstdayofquarter.(market_quarterly_sorted.Date))
	MIN_WEEKLY_OBSERVATIONS_PER_QUARTER = 10
	successful_quarters_count = 0;
	unsuccessful_quarters_count = 0
	previous_quarter_weights = zeros(length(stock_cols))

	# --- Main Loop Execution ---
	for (i, quarter_start) in enumerate(quarters_to_process)
		quarter_end = lastdayofquarter(quarter_start)
		quarter_str = "Q$(Dates.quarter(quarter_start)) $(Dates.year(quarter_start))"
		q_filter(df) = filter(row -> quarter_start <= row.Date <= quarter_end, df)
		log_returns_df = q_filter(stock_weekly_log_returns)
		benchmark_log_returns_df = q_filter(rsp_weekly_log_returns)
		log_returns_in_quarter = Matrix(log_returns_df[:, stock_cols])
		benchmark_log_returns_in_quarter = Matrix(benchmark_log_returns_df[:, ["Benchmark"]])
		num_stocks = size(log_returns_in_quarter, 2)

		println("\n", "="^20, " Processing Quarter: $quarter_str ", "="^20)
		if size(log_returns_in_quarter, 1) < MIN_WEEKLY_OBSERVATIONS_PER_QUARTER
			println("Skipping: insufficient data.");
			continue;
		end

		println("Calculating shrunk covariance matrix...")
		...
		quarterly_shrunk_cov_matrix = if all(var(excess_returns, dims = 1, corrected = false) .< 1e-10)
			println("Warning: Zero variance. Using zero covariance matrix.")
		...
		println("Shrunk covariance matrix calculated.")
		...
		for attempt in 1:max_refinement_attempts
			println("\n--- Attempt $attempt/$max_refinement_attempts | mu = $(round(mu_val, digits=2)) ---")
			try
				...
				pso_objective_func = create_pso_objective_function(
					...
					previous_weights = previous_quarter_weights,
				)
				...
				scipy_obj_wrapper(w) = portfolio_objective_function(convert(Vector{Float64}, w), log_returns_in_quarter, benchmark_log_returns_in_quarter, quarterly_shrunk_cov_matrix, mu_val, diversification_penalty, previous_quarter_weights, num_stocks)
				...
				is_success = slsqp_result["success"]
				final_optimal_weights_unrounded = is_success ? convert(Vector{Float64}, slsqp_result["x"]) : x0
				if !is_success
					println("WARNING: SLSQP refinement did not converge.");
				end
				...
				println("Attempt Result -> Gross IR: $(round(information_ratio, digits=4)), CI: ($(round(ci_lower, digits=4)), $(round(ci_upper, digits=4)))")
				...
				if ci_lower > 0
					println("SUCCESS: Found a robust solution.");
					found_robust_solution = true;
					break;
				end

				println("WARNING: Solution not robust. Increasing risk aversion.");
				mu_val += mu_increment
			catch e
		...
		push!(valid_quarters, quarter_start)
		push!(all_optimal_weights, final_weights)
		push!(all_information_ratios, final_ir)
		bootstrap_results[quarter_start] = Dict("weights" => final_weights, "IR_CI" => final_ci)
		previous_quarter_weights = final_weights
	end

	if !isempty(valid_quarters)
		weights_matrix = hcat(all_optimal_weights...)'
		optimal_weights_df = DataFrame(weights_matrix, Symbol.(stock_cols .* "_Weight"))
		optimal_weights_df.Date = valid_quarters
		ir_df = DataFrame(Date = valid_quarters, Information_Ratio = all_information_ratios)
		ci_df = DataFrame(Date = valid_quarters, IR_CI_Lower = [bootstrap_results[q]["IR_CI"][1] for q in valid_quarters], IR_CI_Upper = [bootstrap_results[q]["IR_CI"][2] for q in valid_quarters])
		final_data_local = innerjoin(ir_df, optimal_weights_df; on = :Date)
		market_data_for_join = select(market_quarterly, Not(r"_Weight$"))
		final_data_local = innerjoin(market_data_for_join, final_data_local; on = :Date)
		final_data_with_ci = innerjoin(final_data_local, ci_df; on = :Date)

		if ENABLE_PLOTTING
			println("\n", "="^25, " GENERATING AND SAVING APSO PLOTS ", "="^25)
			plot_dir = joinpath(pwd(), "Plots", "APSO");
			mkpath(plot_dir)
			weight_columns = [col for col in names(final_data_with_ci) if endswith(string(col), "_Weight")]
			weights_only_df = final_data_with_ci[:, weight_columns]
			plot_df = Matrix(weights_only_df)
			assets = [replace(string(c), "_Weight" => "") for c in weight_columns]
			quarters = string.(final_data_with_ci.Date)
			...
			successful_results = filter(row -> row.IR_CI_Lower > 0, final_data_with_ci)
			unsuccessful_results = filter(row -> row.IR_CI_Lower <= 0, final_data_with_ci)
			...
			p_ci_l = create_summary_distribution_plot(final_data_with_ci.IR_CI_Lower, "Lower CI Bounds (All Quarters)", :blue)
			p_ci_u = create_summary_distribution_plot(final_data_with_ci.IR_CI_Upper, "Upper CI Bounds (All Quarters)", :purple)
...
# --- 3.1 Forecasting Helper Functions (REFACTORED FOR PCA) ---

# This function builds the full feature row for prediction during optimization
function build_prediction_row(new_weights, fixed_pca_features_matrix, weight_cols, pca_feature_names, all_model_features)
	# Create DataFrames for the fixed PCA features and the new weights being tested
	fixed_features_df = DataFrame(fixed_pca_features_matrix, pca_feature_names)
	weights_df = DataFrame(permutedims(new_weights), Symbol.(weight_cols))

	# Combine them to form the base feature set
	base_features_df = hcat(fixed_features_df, weights_df)

	# Create the interaction terms
	interaction_df = DataFrame()
	for pca_col in pca_feature_names, w_col in weight_cols
...
function objective_optimization(new_weights, model, scaler, fixed_pca_features_matrix, weight_cols, pca_feature_names, all_model_features, cov_matrix, lambda_cov, lambda_hhi, target_weights, lambda_prior)
	# Build the complete feature row for the given weights
	prediction_row_df = build_prediction_row(new_weights, fixed_pca_features_matrix, weight_cols, pca_feature_names, all_model_features)
	...
	tracking_error_variance = new_weights' * cov_matrix * new_weights
	hhi = sum(new_weights .^ 2)
	normalized_hhi = length(new_weights) > 1 ? (hhi - 1/length(new_weights)) / (1 - 1/length(new_weights)) : 1.0
	prior_penalty = sum((new_weights .- target_weights) .^ 2)

	return -predicted_ir + (lambda_cov * tracking_error_variance) + (lambda_hhi * normalized_hhi) + (lambda_prior * prior_penalty)
end

# Objective function for NOMAD.jl
function nomad_objective_function(hyperparams::Vector{Float64}, preferences::Dict, model, scaler, fixed_pca_features_matrix, weight_cols, pca_feature_names, all_model_features, cov_matrix, prior_weights)
	...
	res = sp_optimize.minimize(
		...
		prior_weights,
		args = (model, scaler, fixed_pca_features_matrix, weight_cols, pca_feature_names, all_model_features, cov_matrix, lambda_cov, lambda_hhi, prior_weights, lambda_prior),
		method = "SLSQP",
		bounds = [(0, 1) for _ in weight_cols],
		constraints = py"dict"(type = "eq", fun = w -> sum(w) - 1),
	)
	if !res["success"]
		return (false, true, [1e9]);
	end
	w_opt = res["x"]
	# Recalculate IR without penalties for scoring
	clean_ir = -objective_optimization(w_opt, model, scaler, fixed_pca_features_matrix, weight_cols, pca_feature_names, all_model_features, cov_matrix, 0, 0, prior_weights, 0)
	tracking_error = sqrt(abs(w_opt' * cov_matrix * w_opt))
	hhi = sum(w_opt .^ 2)
	score = -(preferences["w_ir"] * clean_ir) + (preferences["w_te"] * tracking_error) + (preferences["w_hhi"] * hhi)
	return (true, true, [score])
end


# --- 3.2 Main Forecasting Orchestrator (REFACTORED WITH PCA) ---
function run_forecasting_stage(final_features_df::DataFrame, prices_weekly_df::DataFrame;
	...
	# --- Stage 3.1: Loading All Necessary Data ---
	println("\n--- Stage 3.1: Loading Data ---")
	mkpath(FORECAST_PLOTS_DIR)
	local df_full
	try
		df_full = final_features_df
		println("Successfully loaded data for forecasting stage.")
	catch e
		println("ERROR: Could not process in-memory data. Details: $e")
		return
	end

	# --- Data Separation ---
	all_features = filter(c -> c != "Date" && c != TARGET_VARIABLE, names(df_full))
	weight_cols = filter(col -> endswith(col, "_Weight"), all_features)
	...
	# --- Stage 3.2: Calculating Ledoit-Wolf Shrunk Covariance Matrix ---
	println("\n--- Stage 3.2: Calculating Ledoit-Wolf Shrunk Covariance Matrix ---")
	local shrunk_cov_matrix
	try
		benchmark_col_name = benchmark
		if benchmark_col_name ∉ names(prices_weekly_df)
			println("ERROR: Benchmark '$(benchmark_col_name)' not found in price data.")
			return
		end

		stock_cols_returns = [col for col in names(prices_weekly_df) if col != benchmark_col_name && col != "Date"]
		log_ret(p) = [NaN; log.(p[2:end] ./ p[1:(end-1)])]
		clean_val(x) = !isfinite(x) ? 0.0 : x
		stock_weekly_log_returns = DataFrame(Date = prices_weekly_df.Date)
		for col in stock_cols_returns
			prices = coalesce.(prices_weekly_df[:, col], NaN)
			stock_weekly_log_returns[!, col] = clean_val.(log_ret(prices))
		end
		benchmark_prices = coalesce.(prices_weekly_df[:, benchmark_col_name], NaN)
		...
		asset_names_base = [replace(col, "_Weight" => "") for col in weight_cols]

		valid_asset_names = [name for name in asset_names_base if name in names(stock_returns_filtered)]

		temp_stock_returns = copy(stock_returns_filtered);
		temp_stock_returns.Quarter = firstdayofquarter.(temp_stock_returns.Date)
		quarterly_stock_returns_df = combine(groupby(temp_stock_returns, :Quarter), [Symbol(c) => sum => Symbol(c) for c in valid_asset_names]...)

		benchmark_returns_filtered = filter(row -> min_date <= row.Date <= max_date, rsp_weekly_log_returns)
		temp_bench_returns = copy(benchmark_returns_filtered);
		temp_bench_returns.Quarter = firstdayofquarter.(temp_bench_returns.Date)
		quarterly_bench_returns_df = combine(groupby(temp_bench_returns, :Quarter), :Benchmark => sum => :Benchmark)

		quarterly_returns_aligned = innerjoin(quarterly_stock_returns_df, quarterly_bench_returns_df, on = :Quarter)

		asset_returns_matrix = Matrix(quarterly_returns_aligned[:, Symbol.(valid_asset_names)])
		benchmark_returns_vector = quarterly_returns_aligned.Benchmark
		excess_returns_matrix = asset_returns_matrix .- benchmark_returns_vector

		lw = sklearn_covariance.LedoitWolf()
		lw.fit(excess_returns_matrix)
		shrunk_cov_matrix = convert(Matrix{Float64}, lw."covariance_")
		println("Successfully calculated shrunk covariance matrix.")
	catch e
		println("ERROR: Failed to calculate shrunk covariance matrix. Details: $e")
		return
	end

	# --- Stage 3.3: PCA on Non-Weight Features and Interaction Engineering ---
	println("\n--- Stage 3.3: Applying PCA and Engineering Interaction Features ---")

	X_history_non_weight = Matrix(df_history[:, Symbol.(non_weight_features)])

	if ENABLE_PLOTTING
		# --- PLOT: Correlation Matrix of Original Features ---
		corr_matrix_before_pca = cor(X_history_non_weight)
		p_corr_before = Plots.heatmap(
			non_weight_features, non_weight_features, corr_matrix_before_pca,
			c = :coolwarm, clims = (-1, 1),
			title = "Original Non-Weight Features",
			xrotation = 90,
			tickfontsize = 6, bottom_margin = 100*Plots.px, left_margin = 100*Plots.px,
		)
	end
	...
	if ENABLE_PLOTTING
		...
		corr_matrix_after_pca = cor(X_history_pca)
		p_corr_after = Plots.heatmap(
			pca_feature_names, pca_feature_names, corr_matrix_after_pca,
			c = :coolwarm, clims = (-1, 1),
			title = "Principal Components (After PCA)",
			xrotation = 90,
		)

		# Combine and save the correlation plots
		fig_corr_subplots = Plots.plot(
			p_corr_before, p_corr_after,
			layout = (1, 2),
			plot_title = "Feature Correlation Before and After PCA",
			size = (1600, 700),
		)
		Plots.savefig(fig_corr_subplots, joinpath(FORECAST_PLOTS_DIR, "correlation_pca_subplots.png"))
		println("Saved Before/After PCA correlation heatmaps.")
	end

	X_history_pca_df = DataFrame(X_history_pca, Symbol.(pca_feature_names))
	X_history_weights_df = df_history[:, Symbol.(weight_cols)]
	...
	X_interactive[!, All()] = coalesce.(X_interactive[!, All()], 0.0)
	...
	# --- Stage 3.5: Evaluating Model Performance on Validation Set ---
	println("\n--- Stage 3.5: Evaluating Model Performance on Validation Set ---")
	val_predictions = ridge_cv_model.predict(X_val_scaled)
	mse = mean((y_val .- val_predictions) .^ 2)
	mae = mean(abs.(y_val .- val_predictions))
	denominator = abs.(y_val) .+ abs.(val_predictions)
	smape_terms = ifelse.(denominator .== 0, 0.0, 2 .* abs.(val_predictions .- y_val) ./ (denominator .+ epsilon))
	smape = mean(smape_terms) * 100
	directional_accuracy = mean(sign.(val_predictions) .== sign.(y_val)) * 100
	...
	# --- Stage 3.6: Training Final Model on All Data ---
	println("\n--- Stage 3.6: Training Final Model on All Data ---")
	...
	println("Final Ridge model trained on all historical data.")

	# --- Prepare Prediction Point Data ---
	...
	# --- Stage 3.7: Tuning Hyperparameters with NOMAD.jl ---
	println("\n--- Stage 3.7: Tuning Hyperparameters with NOMAD.jl ---")
	preferences = Dict("w_ir" => 0.4, "w_te" => 0.3, "w_hhi" => 0.3)
	lower_bounds, upper_bounds, x0 = [0.01, 0.01, 0.01], [5.0, 5.0, 5.0], [0.5, 0.5, 0.5]
	opts = NOMAD.NomadOptions(max_bb_eval = nomad_max_evals, display_degree = 0)

	model_coeffs = DataFrame(Feature = all_model_features, Coeff = final_model.coef_)
	main_prior_weights = [abs(get(model_coeffs[model_coeffs.Feature .== w, :Coeff], 1, 0.0)) for w in weight_cols]
	main_prior_weights ./= (sum(main_prior_weights) + epsilon)

	nomad_obj_func = (x) -> nomad_objective_function(x, preferences, final_model, model_scaler, fixed_pca_features_matrix, weight_cols, pca_feature_names, all_model_features, shrunk_cov_matrix, main_prior_weights)
	...
	# --- Stage 3.7B: Bootstrapping to get Weight Statistics ---
	println("\n--- Stage 3.7B: Bootstrapping to get Weight Statistics ---")
	bootstrap_weights_matrix = Matrix{Float64}(undef, length(weight_cols), n_bootstrap)
	...
	for i in 1:n_bootstrap
		...
		bootstrap_prior_weights = [abs(get(model_coeffs_sample[model_coeffs_sample.Feature .== w, :Coeff], 1, 0.0)) for w in weight_cols]
		bootstrap_prior_weights ./= (sum(bootstrap_prior_weights) + epsilon)

		bootstrap_opt_result = sp_optimize.minimize(objective_optimization, bootstrap_prior_weights,
			args = (bootstrap_model, scaler_sample, fixed_pca_features_matrix, weight_cols, pca_feature_names, all_model_features, shrunk_cov_matrix, best_hyperparams[1], best_hyperparams[2], bootstrap_prior_weights, best_hyperparams[3]),
			method = "SLSQP", bounds = [(0, 1) for _ in weight_cols], constraints = py"dict"(type = "eq", fun = w -> sum(w) - 1))

		bootstrap_weights_matrix[:, i] = bootstrap_opt_result["success"] ? bootstrap_opt_result["x"] : NaN
		next!(prog)
	end
	...
	# --- Stage 3.8: Visualize & Display Final "Best" Portfolio ---
	println("\n--- Stage 3.8: Generating Final Results and Plots ---")
	final_opt_result = sp_optimize.minimize(
		objective_optimization,
		main_prior_weights,
		args = (final_model, model_scaler, fixed_pca_features_matrix, weight_cols, pca_feature_names, all_model_features, shrunk_cov_matrix, best_hyperparams[1], best_hyperparams[2], main_prior_weights, best_hyperparams[3]),
		method = "SLSQP",
		bounds = [(0, 1) for _ in weight_cols],
		constraints = py"dict"(type = "eq", fun = w -> sum(w) - 1),
	)
	...
	if !final_opt_result["success"]
		println("ERROR: Final optimization with best parameters failed.")
		return
	end
	optimal_weights = final_opt_result["x"]
	asset_names = [replace(w, "_Weight" => "") for w in weight_cols]

	if ENABLE_PLOTTING
		# --- Plot 1: Feature Importance Histogram ---
		...
		if !isempty(feature_importance_df.Importance) && any(x -> x > 0, feature_importance_df.Importance)
			...
			histogram_df.SortKey = get_sort_key.(histogram_df.PercentileBin);
		...
		# --- Plot 2: Portfolio Allocation ---
		plot_df = DataFrame(Asset = asset_names, CurrentWeight = optimal_weights, AvgWeight = avg_weights, CIMargin = ci_margin)
	...
	final_ir = -objective_optimization(optimal_weights, final_model, model_scaler, fixed_pca_features_matrix, weight_cols, pca_feature_names, all_model_features, shrunk_cov_matrix, 0, 0, main_prior_weights, 0)
	@printf "Model's Predicted Information Ratio: %.4f\n" final_ir
	@printf "Portfolio's Predicted Tracking Error:  %.6f\n" sqrt(abs(optimal_weights' * shrunk_cov_matrix * optimal_weights))
	@printf "Portfolio's HHI (Concentration):       %.4f\n" sum(optimal_weights .^ 2)

	println("\n--- Final Optimal Portfolio Weights ---")
	results_text_df = sort(DataFrame(Asset = asset_names, Weight = optimal_weights), :Weight, rev = true)
	...
	results_bootstrap_df = sort(DataFrame(Asset = asset_names, Weight = avg_weights, CIMargin = ci_margin), :Weight, rev = true)
	results_bootstrap_df_filtered = filter(row -> row.Weight > 1e-4, results_bootstrap_df)
	results_bootstrap_df_filtered.Weight = [@sprintf("%.2f%%", w * 100) for w in results_bootstrap_df_filtered.Weight]
	results_bootstrap_df_filtered.CIMargin = [@sprintf("±%.2f%%", e * 100) for e in results_bootstrap_df_filtered.CIMargin]
	rename!(results_bootstrap_df_filtered, :CIMargin => Symbol("95% CI Margin"))
	println(results_bootstrap_df_filtered)
	println("="^50 * "\n")

	return results_bootstrap_df
end
